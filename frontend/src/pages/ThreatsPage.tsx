...
function TopoNode({ data }: { data: { label: string; nodeType: string; hostname: string; customer: string } }) {
  ...
  const Icon = cfg.icon;
  return (
    <div className="rounded-lg px-3 py-2 min-w-[120px] text-center border relative" style={{ backgroundColor: "var(--theme-card-bg)", borderColor: cfg.color + "40" }}>
      <Handle type="target" position={Position.Top} style={{ background: "var(--theme-text-dim)", border: "none", width: 6, height: 6 }} />
      <Handle type="target" position={Position.Left} id="left" style={{ background: "var(--theme-text-dim)", border: "none", width: 6, height: 6 }} />
      ...
      <Handle type="source" position={Position.Bottom} style={{ background: "var(--theme-text-dim)", border: "none", width: 6, height: 6 }} />
      <Handle type="source" position={Position.Right} id="right" style={{ background: "var(--theme-text-dim)", border: "none", width: 6, height: 6 }} />
    </div>
  );
}
...
export function ThreatsPage() {
  ...
  const onConnect: OnConnect = useCallback(async (conn) => {
    ...
    try {
      ...
      setRfEdges(eds => addEdge({ ...conn, id: String(link.id), label: "lan", style: { stroke: "var(--theme-text-dim)", strokeWidth: 1.5 }, labelStyle: { fill: "var(--theme-text-muted)", fontSize: 9 }, labelBgStyle: { fill: "var(--theme-surface-base)", fillOpacity: 0.8 } }, eds));
    } catch {}
  }, []);
  ...
  const onEdgeClick = useCallback((_: React.MouseEvent, edge: Edge) => {
    ...
    const clr = edge.style?.stroke as string || "var(--theme-text-dim)";
  ...
  const importTopologyJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!data.nodes || !Array.isArray(data.nodes)) { setAlertMsg("Invalid topology JSON"); return; }
      ...
      setAlertMsg(`Imported ${data.nodes.length} nodes`);
      await loadData();
    } catch (err) { setAlertMsg("Import failed: " + String(err)); }
  ...
  const exportScreenshot = async () => {
    ...
    if (!el) { setAlertMsg("No graph to capture"); return; }
    try {
      ...
      a.click();
    } catch (e) { setAlertMsg("Screenshot failed: " + String(e)); }
  };
    ...
    <div className="relative w-full flex flex-col h-[calc(100vh-3rem)] sm:h-[calc(100vh-3.5rem)]" style={{ background: "var(--theme-surface-base)" }}>
      ...
      {replayOpen && mode === "map" && (
        <div className="absolute top-16 left-4 z-[1000] px-4 py-3 rounded-lg flex items-center gap-3 text-xs" style={{ backgroundColor: "var(--theme-nav-bg)", border: "1px solid var(--theme-surface-border)" }}>
          ...
          <input type="datetime-local" value={replayEnd} onChange={e => setReplayEnd(e.target.value)} className="px-2 py-1 rounded text-xs" style={{ backgroundColor: "var(--theme-card-bg)", border: "1px solid var(--theme-surface-border)", color: "var(--theme-text-primary)" }} />
          <span style={{ color: "var(--theme-text-muted)" }}>Asset</span>
          <input type="text" value={replayHostname} onChange={e => setReplayHostname(e.target.value)} placeholder="any" className="px-2 py-1 rounded text-xs w-32" style={{ backgroundColor: "var(--theme-card-bg)", border: "1px solid var(--theme-surface-border)", color: "var(--theme-text-primary)" }} />
          <button onClick={loadReplay} className="px-3 py-1 rounded text-xs font-medium" style={{ backgroundColor: "var(--theme-surface-raised)", border: "1px solid var(--theme-surface-border)", color: "var(--theme-text-primary)" }}>Load</button>
          {replayData.length > 0 && (
            <>
              {replayPlaying ? (
              ...
              <span className="font-mono" style={{ color: "var(--theme-text-muted)" }}>{replayIndex}/{replayData.length}</span>
            </>
          )}
        </div>
      )}
...
function Fld({ label, value, onChange, placeholder, type = "text" }: { label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <div>
      ...
      <input type={type} value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder} className="w-full text-xs px-3 py-2 rounded-lg outline-none" style={{ backgroundColor: "var(--theme-card-bg)", border: "1px solid var(--theme-surface-border)", color: "var(--theme-text-primary)" }} />
    </div>
  );
}
