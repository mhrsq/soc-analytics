...
async def get_profiles(user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    """Get all dashboard profiles for the current user."""
    result = await db.execute(
        select(DashboardProfile)
        .where(DashboardProfile.user_id == user.id)
...
@router.put("/profiles")
async def save_profiles(body: BulkSave, user: User = Depends(require_auth), db: AsyncSession = Depends(get_db)):
    """Bulk save all profiles for the current user (replaces all)."""
    # Delete existing profiles for this user
    existing = await db.execute(
        select(DashboardProfile).where(DashboardProfile.user_id == user.id)
    )
